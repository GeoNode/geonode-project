...
IPAddress"
    ...
    try:
        ...
        logging.info(
            "NGINX container is not running maybe exited! Running\
containers are:{0}".format(
                containers
            )
        )
except KeyError as ke:
    logging.info(
        "There has been a problem with the docker\
network which has raised the following exception: {0}".format(
            ke
        )
    )
else:
