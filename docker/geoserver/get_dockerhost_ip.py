...
def _docker_host_ip():
    ...
    if len(ip_list) > 1:
        logging.info(
            "Docker daemon is running on more than one \
address {0}".format(
                ip_list
            )
        )
        logging.info("Only the first address:{0} will be returned!".format(ip_list[0]))
    else:
        logging.info(
            "Docker daemon is running at the following \
address {0}".format(
                ip_list[0]
            )
        )
