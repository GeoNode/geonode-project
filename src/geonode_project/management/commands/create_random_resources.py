        ...
        for i in range(1, opts["count"] + 1):
            ...
            try:
                with transaction.atomic():
                    if rtype == "document":
                        # Document has no FileField on current GeoNode master (files go through
                        # a LocalAsset/Link instead) — fall back to doc_url for a metadata-only doc.
                        doc = Document(
                            title=title,
                            abstract=abstract,
                            owner=owner,
                        ...
                        file_field = next(
                            (f.name for f in Document._meta.get_fields() if isinstance(f, FileField)), None
                        )
                        if file_field:
                            content = f"Synthetic test document {uuid.uuid4()}\n{abstract}".encode()
                            getattr(doc, file_field).save(f"{random_suffix()}.txt", ContentFile(content), save=False)
                        elif hasattr(doc, "doc_url"):
                            doc.doc_url = f"https://example.com/fake-doc-{random_suffix()}.txt"
                        doc.save()
                        resource = doc

                    elif rtype == "map":
                        # zoom/center_x/center_y/projection don't exist on current GeoNode master's
                        # Map model — only pass kwargs the running version actually has.
                        map_field_names = {f.name for f in Map._meta.get_fields()}
                        desired = dict(title=title, abstract=abstract, owner=owner, category=category)
                        m = Map(**{k: v for k, v in desired.items() if k in map_field_names})
                        m.save()
                        resource = m
