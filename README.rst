...
Step 2. Run the following command in a terminal. Remember to replace the name of the project from `{{project_name}}` to whatever you want, for example `awesomegeonode`.::
    ...
    docker run -v `pwd`:/usr/src/app terranodo/django:geonode django-admin.py startproject {{project_name}} --template=https://github.com/piensa/geonode-project/archive/docker.zip -epy,rst,yml
    cd {{project_name}}
